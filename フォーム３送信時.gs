...
function submitReception(e) {
  ...
  var formResponses = mielApplication.getSheetByName('フォーム(3)').getRange(2,1,lastRow-1,8).getValues();
...
function checkCount(mielForm,mielApplication,formResponses){
  ...
  var isClosed = false;
  //最大申込数を確認
  var max = mielApplication.getSheetByName('【管理】').getRange("C6").getValue();
...
function updateInformation(mielForm, mielApplication, count, isClosed){
  //管理シートを更新する
  mielApplication.getSheetByName('【管理】').getRange("D6").setValue(count);
  mielApplication.getSheetByName('【管理】').getRange("E6").setValue(Utilities.formatDate(new Date(),'Asia/Tokyo', 'yyyy年MM月dd日 HH:mm'));
  if (isClosed) {
    mielApplication.getSheetByName('【管理】').getRange("F6").setValue("売切");
  }
}
...
function sendMailtoMiel(mielForm,mielApplication,newResponse,mielEMail){
  ...
  var max = mielApplication.getSheetByName('【管理】').getRange("C6").getValue();
  var current = mielApplication.getSheetByName('【管理】').getRange("D6").getValue();
  var nowDate = mielApplication.getSheetByName('【管理】').getRange("E6").getValue();
  var isClosed = mielApplication.getSheetByName('【管理】').getRange("F6").getValue();
  var mailAddress = mielApplication.getSheetByName('【管理】').getRange("H6").getValue();
  ...
  console.log('管理者送信メール' + mailMessage + mailMessage2);
