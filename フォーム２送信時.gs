...
function checkCount(mielForm,mielApplication,formResponses){
  ...
  var isClosed = false;
  //最大申込数を確認
  var max = mielApplication.getSheetByName('【管理】').getRange("C5").getValue();
...
function updateInformation(mielForm, mielApplication, count, isClosed){
  //管理シートを更新する
  mielApplication.getSheetByName('【管理】').getRange("D5").setValue(count);
  mielApplication.getSheetByName('【管理】').getRange("E5").setValue(Utilities.formatDate(new Date(),'Asia/Tokyo', 'yyyy年MM月dd日 hh:mm'));
  if (isClosed) {
    mielApplication.getSheetByName('【管理】').getRange("F5").setValue("売切");
  }
}
...
function sendMailtoMiel(mielForm,mielApplication,newResponse,mielEMail){
  ...
  var max = mielApplication.getSheetByName('【管理】').getRange("C5").getValue();
  var current = mielApplication.getSheetByName('【管理】').getRange("D5").getValue();
  var nowDate = mielApplication.getSheetByName('【管理】').getRange("E5").getValue();
  var isClosed = mielApplication.getSheetByName('【管理】').getRange("F5").getValue();
